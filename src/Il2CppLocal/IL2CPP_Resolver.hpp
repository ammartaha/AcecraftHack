    ...
    namespace UnityAPI
    {
            ...
            switch (m_ExportObfuscation)
            {
                case m_eExportObfuscationType::ROT:
                {
                    if (m_ROTObfuscationValue == -1) // Bruteforce
                    {
                        for (int i = 1; 26 > i; ++i)
                        {
                            void* m_Return = dlsym(Globals.m_GameFramework, &Unity::Obfuscators::ROT_String(m_Name, i)[0]);
                            if (m_Return)
                            {
                    ...
                    return dlsym(Globals.m_GameFramework, &Unity::Obfuscators::ROT_String(m_Name, m_ROTObfuscationValue)[0]);
                }
                default:
                    return dlsym(Globals.m_GameFramework, m_Name);
            }
        ...
        bool Initialize()
        {
            ...
            std::unordered_map<const char*, void**> m_ExportMap =
            {
                ...
                { IL2CPP_ASSEMBLY_GET_IMAGE,                    &Functions.m_AssembliesGetImage },
                { IL2CPP_IMAGE_GET_NAME,                    &Functions.m_ImageGetName },
            };
    ...
    */
    bool Initialize(bool m_WaitForModule = false, int m_MaxSecondsWait = 60, const char *dir = "UnityFramework")
    {
        ...
        if (m_WaitForModule)
        {
            ...
            while (!Globals.m_GameFramework)
            {
                if (m_SecondsWaited >= m_MaxSecondsWait)
                    return false;

                Globals.m_GameFramework = dlopen(dir, RTLD_LAZY);
                ++m_SecondsWaited;
        ...
        if (Globals.m_GameFramework == nullptr)
            return false;
